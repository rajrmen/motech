...
public class ServerEventRelay implements EventRelay {
    ...
    public void relayEvent(MotechEvent event) {
        relayMotechEvent(event, eventListenerRegistry);
    }

    /**
     * Relays an event error to all the error listeners for that event.
     *
     * @param event event being relayed
     */
    public void relayErrorEvent(MotechEvent event) {
        relayMotechEvent(event, eventErrorListenerRegistry);
    }

    private void relayMotechEvent(MotechEvent event, EventListenerRegistry registry) {
        ...
        Set<EventListener> listeners = registry.getListeners(event.getSubject());
