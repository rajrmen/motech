...
class EventListenerTree
{
	...
	public void removeAllListeners(String beanName) {
		...
		for (Iterator<EventListenerTree> listenerIterator 
				= children.iterator(); listenerIterator.hasNext(); ) {
	...
	public boolean containsListenersForBeanName(String beanName) {

		if (listeners != null) {
			for (Iterator<EventListener> listenerIterator 
					= listeners.iterator(); listenerIterator.hasNext(); ) {
		...
		if (wildcardListeners != null) {
			for (Iterator<EventListener> listenerIterator 
					= wildcardListeners.iterator(); listenerIterator.hasNext(); ) {
				EventListener nextListener = listenerIterator.next();
				if (nextListener.getIdentifier().equals(beanName)) {
					listenerIterator.remove();
					return true;
