...
public class NotifierTest {
    ...
    public void shouldSendNotification() throws Exception {
        ...
        when(motechSettings.getServerUrl()).thenReturn("serverurl");

        Notifier notifierSpy = spy(notifier);

        ArgumentCaptor<Map> velocityArgumentCaptor = ArgumentCaptor.forClass(Map.class);
        doReturn(text).when(notifierSpy).mergeTemplateIntoString(velocityArgumentCaptor.capture());

        notifierSpy.send(statusMessage, "recipients");
    ...
    @Test
    public void shouldAddSchemeWhenHttpIsNotPartOfTheUrl() {
        when(motechSettings.getServerUrl()).thenReturn("serverurl");
        assertThat(notifier.messagesUrl(), IsEqual.equalTo("http://serverurl/module/server/?moduleName=admin#/messages"));
    }

    @Test
    public void shouldNotAddSchemeWhenHttpIsPartOfTheUrl() {
        when(motechSettings.getServerUrl()).thenReturn("https://serverurl");
        assertThat(notifier.messagesUrl(), IsEqual.equalTo("https://serverurl/module/server/?moduleName=admin#/messages"));
    }

    @Test
    public void shouldJustReturnUrlPathWhenServerUrlIsNotGiven() {
        when(motechSettings.getServerUrl()).thenReturn(null);
        assertThat(notifier.messagesUrl(), IsEqual.equalTo("/module/server/?moduleName=admin#/messages"));
    }
