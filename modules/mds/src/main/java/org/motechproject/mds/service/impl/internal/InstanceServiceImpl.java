...
public class InstanceServiceImpl extends BaseMdsService implements InstanceService {
    ...
    public Object createInstance(EntityDto entityDto, List<FieldRecord> fieldRecords) {
        try {
            ...
            //repository.setPersistenceManagerFactory(allEntityMappings.getPersistenceManagerFactory());
    ...
    public List<?> getEntityRecordsPaged(Long entityId, Integer page, Integer rows) {
        ...
        String className = ClassName.getSimpleName(allEntityMappings.getEntityById(entityId).getClassName());
        try {
            ...
            MotechDataRepository repository = (MotechDataRepository) repositoryClass.newInstance();
            //PersistenceManagerFactory pmf = allEntityMappings.getPersistenceManagerFactory();

            //repository.setPersistenceManagerFactory(pmf);
...
//            if (!settings.getSortColumn().isEmpty() && !list.isEmpty()) {
//                Collections.sort(
//                        list, new EntityRecordComparator(sortAscending, settings.getSortColumn())
//                );
//            }
