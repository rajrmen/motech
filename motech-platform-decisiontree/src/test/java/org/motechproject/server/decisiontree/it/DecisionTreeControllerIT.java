...
public class DecisionTreeControllerIT extends SpringIntegrationTest {
    ...
    public void shouldReturnVXML() throws Exception {
        ...
        final String vmlUrl = "http://localhost:8080/vxml/node?type=vxml&pId=asd&ln=en&tNm=someTree&trP=Lw==";
    ...
    public void shouldReturnVerboiceML() throws Exception {
        server.join();
        HttpClient client = new DefaultHttpClient();
        final String vmlUrl = "http://localhost:8080/vxml/node?type=verboice&pId=asd&ln=en&tNm=someTree&trP=Lw==";
        final String response = client.execute(new HttpGet(vmlUrl), new BasicResponseHandler());
        Assert.assertTrue(!response.contains("<vxml version=\"2.1\" xmlns=\"http://www.w3.org/2001/vxml\">"));
    }

    @Test
    public void shouldReturnVerboiceMLWithNavigation() throws Exception {
        HttpClient client = new DefaultHttpClient();
        final String vmlUrl = "http://localhost:8080/vxml/node?type=verboice&pId=asd&ln=en&tNm=someTree&trP=Lw==";
        final String response = client.execute(new HttpGet(vmlUrl), new BasicResponseHandler());
        Assert.assertTrue(response.contains("<Redirect method=\"POST\">/decisiontree/node?type=verboice&amp;pId=asd&amp;ln=en&amp;tNm=OtherTree</Redirect>"));
        server.join();
    }
