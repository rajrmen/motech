...
if [ "$#" -eq 4 -a "$1" = add -a $3 -eq $3 2>/dev/null -a $4 -eq $4 2>/dev/null ]; then
	...
	rm -rf /var/lib/motech/motech-$2/data/config/motech-settings.conf
	echo "motech.app.name=$2" | tee /var/lib/motech/motech-$2/data/config/motech.properties > /dev/null
	...
	echo 1.Add Tenant: manage_Motech_Instances.sh add tenant_name connector_port_number shutdown_port_number 
	echo 2.Remove Tenant: manage_Motech_Instances.sh remove tenant_name
	echo 3.Update Tenant: manage_Motech_Instances.sh update tenant_name
