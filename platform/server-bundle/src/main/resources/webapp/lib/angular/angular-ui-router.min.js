...
function $StateRefDirective($state) {
  return {
    ...
    link: function(scope, element, attrs) {
      var ref = attrs.uiSref.match(/^([^(]+?)\s*(\((.*)\))?$/);
      if (!ref || ref.length !== 4) throw new Error("Invalid state ref '" + attrs.uiSref + "'");

      var state = ref[1], paramExpr = ref[3] || null, params = null, url = null;
      ...
      var update = function(newVal) {
        ...
        try {
          element[0][attr] = $state.href(state, params, { lossy: true });
        } catch (e) {
          nav = false;
        }
      };
      ...
      if (paramExpr) {
        scope.$watch(paramExpr, function(newVal, oldVal) {
          if (newVal !== oldVal) update(newVal);
        }, true);
        params = scope.$eval(paramExpr);
      }
      element[0][attr] = update();
